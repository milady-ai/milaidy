...
const STATUS_TONES: Record<StatusTone, StatusToneStyles> = {
  success: {
    badge: "text-[var(--ok,#16a34a)] border border-[var(--ok,#16a34a)]/30 bg-[var(--ok,#16a34a)]/10",
...
export function StatusBadge({
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-[10px] font-bold uppercase ${toneStyles.badge} ${className}`}>
      {withDot && <span className={`w-1.5 h-1.5 rounded-full ${toneStyles.dot}`} />}
      {label}
    </span>
  );
}
...
export function StatusDot({
  ...
  const toneStyles = STATUS_TONES[tone];
  return <span className={`inline-block w-2 h-2 rounded-full ${toneStyles.dot} ${className}`} />;
}
...
export function StatCard({
  ...
  return (
    <div className="flex flex-col items-center justify-center p-3 border border-[var(--border)] bg-[var(--bg)] min-w-[80px]">
      ...
      <div className="text-[10px] text-[var(--muted)] uppercase tracking-wide mt-0.5">{label}</div>
    </div>
  );
}
