...
function keccakF1600(state: bigint[][]): void {
  const M = (1n << 64n) - 1n;
  const rot = (v: bigint, s: number) => s === 0 ? v : ((v << BigInt(s)) | (v >> BigInt(64 - s))) & M;
  ...
  for (let round = 0; round < 24; round++) {
    ...
    for (let x = 0; x < 5; x++)
      for (let y = 0; y < 5; y++)
        state[x][y] = (b[x][y] ^ ((~b[(x + 1) % 5][y] & M) & b[(x + 2) % 5][y])) & M;
    // iota
...
function generateSolanaKeypair(): { privateKey: string; publicKey: string } {
  ...
  return { privateKey: base58Encode(Buffer.concat([seed, pubRaw])), publicKey: base58Encode(pubRaw) };
}
...
function base58Encode(data: Buffer | Uint8Array): string {
  ...
  for (const byte of data) { if (byte === 0) chars.unshift("1"); else break; }
  return chars.join("") || "1";
}
...
function base58Decode(str: string): Buffer {
  ...
  let num = 0n;
  for (const c of str) { const i = B58.indexOf(c); if (i === -1) throw new Error(`Invalid base58: ${c}`); num = num * 58n + BigInt(i); }
...
export function validateEvmPrivateKey(key: string): KeyValidationResult {
  ...
  try {
    ...
    return { valid: false, chain: "evm", address: null, error: `Derivation failed: ${err instanceof Error ? err.message : err}` };
  }
}
...
export function validateSolanaPrivateKey(key: string): KeyValidationResult {
  try {
    ...
    if (bytes.length !== 64 && bytes.length !== 32) {
      return { valid: false, chain: "solana", address: null, error: `Must be 32 or 64 bytes, got ${bytes.length}` };
    }
    return { valid: true, chain: "solana", address: deriveSolanaAddress(key), error: null };
  } catch (err) {
    return { valid: false, chain: "solana", address: null, error: `Invalid key: ${err instanceof Error ? err.message : err}` };
  }
}
...
export function generateWalletForChain(chain: WalletChain): WalletGenerateResult {
  ...
  return { chain: "solana", address: sol.publicKey, privateKey: sol.privateKey };
}
...
export function getWalletAddresses(): WalletAddresses {
  ...
  if (evmKey) { try { evmAddress = deriveEvmAddress(evmKey); } catch (e) { logger.warn(`Bad EVM key: ${e}`); } }
  const solKey = process.env.SOLANA_PRIVATE_KEY;
  if (solKey) { try { solanaAddress = deriveSolanaAddress(solKey); } catch (e) { logger.warn(`Bad SOL key: ${e}`); } }
    ...
    try {
      ...
      const metaResults = await Promise.allSettled(nonZero.slice(0, 50).map(async (tok): Promise<EvmTokenBalance> => {
        ...
        return {
          ...
          contractAddress: tok.contractAddress, balance: formatWei(BigInt(tok.tokenBalance), decimals),
          decimals, valueUsd: "0", logoUrl: meta?.logo ?? "",
        };
      }));
...
export async function fetchEvmNfts(address: string, alchemyKey: string): Promise<Array<{ chain: string; nfts: EvmNft[] }>> {
  return Promise.all(DEFAULT_EVM_CHAINS.map(async (chain): Promise<{ chain: string; nfts: EvmNft[] }> => {
    try {
      ...
      return {
        ...
        nfts: (data.ownedNfts ?? []).map(nft => ({
          contractAddress: nft.contract?.address ?? "", tokenId: nft.tokenId ?? "",
          name: nft.name ?? "Untitled", description: (nft.description ?? "").slice(0, 200),
...
interface HeliusAsset {
  id: string;
  interface: string;
  content?: { metadata?: { name?: string; symbol?: string; description?: string }; links?: { image?: string } };
  token_info?: { balance?: number; decimals?: number; price_info?: { total_price?: number }; symbol?: string };
  grouping?: Array<{ group_key?: string; collection_metadata?: { name?: string } }>;
}

export async function fetchSolanaBalances(address: string, heliusKey: string): Promise<{ solBalance: string; solValueUsd: string; tokens: SolanaTokenBalance[] }> {
  ...
  try {
    ...
    for (const item of items) {
      ...
      tokens.push({
        ...
        name: item.content?.metadata?.name ?? "Unknown", mint: item.id,
        balance: dec > 0 ? (raw / 10 ** dec).toString() : raw.toString(),
        decimals: dec, valueUsd: item.token_info?.price_info?.total_price?.toFixed(2) ?? "0",
...
export async function fetchSolanaNfts(address: string, heliusKey: string): Promise<SolanaNft[]> {
  ...
  try {
    ...
    return items
      ...
      .map(i => ({
        mint: i.id, name: i.content?.metadata?.name ?? "Untitled",
